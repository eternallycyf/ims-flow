import { Logic, Context, createLogic, FLOW_SHAPES } from '../src/logic';
...
describe('Logic', () => {
  ...
  it('should register and invoke a start node', async () => {
    const logic = new Logic({
      dsl: {
        cells: [
          { id: 'start', shape: FLOW_SHAPES.START, data: { trigger: 'init', configData: { a: 1, b: 2 } } },
        ],
      },
      nodeFns: {
        start: (ctx) => {
...
describe('createLogic', () => {
  it('should create a Logic with DSL', async () => {
    const logic = createLogic({
      dsl: {
        cells: [
          { id: 'n1', shape: FLOW_SHAPES.START, data: { trigger: 'test', configData: { value: 7 } } },
        ],
      },
      nodeFns: {
        n1: (ctx) => {
